...
sub _initialize_s3_or_die($)
{
    ...
    eval {
        ...
        {
            LOGDIE(
                "Unable to initialize Net::Amazon::S3 instance with access key '" . $self->_config_access_key_id . "'." );
        }
        ...
        eval {
            ...
            if ( $self->_config_head_before_putting or ( !$self->_config_overwrite ) )
            {
                if ( $self->head( $filename ) )
                {
                    ...
                    {
                        WARN( "File '$filename' already exists, " . "will store a new version or overwrite " .
